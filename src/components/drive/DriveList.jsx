...
function RowMenu({ items }) {
  ...
  return (
    <div ref={boxRef} style={{ position: "relative" }}>
      ...
      {open ? (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 4px)", minWidth: 160, background: "rgba(16,16,20,0.98)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 6, boxShadow: "0 14px 32px rgba(0,0,0,0.42)", zIndex: 50, display: "grid", gap: 4 }}>
          {items.map((item, idx) => (
    ...
    <div>
      {notes.map((n) => (
        ...
        >
          ...
          <RowMenu
            items={[
              { label: "Open", onClick: () => onSelect(n.id) },
              { label: "Rename", onClick: () => onRename(n.id) },
              { label: "Move", onClick: () => onMove(n.id) },
              { label: "Delete", onClick: () => onDelete(n.id) },
            ]}
          />
        </div>
      ))}
