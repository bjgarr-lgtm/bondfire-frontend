...
export default function DriveSidebar({ folders, currentFolder, setCurrentFolder, onNewFolder, onRename, onDelete, tags, selectedTag, setSelectedTag, templates = [], onApplyTemplate, onDeleteTemplate, onNewFromTemplate }) {
  const [templatesOpen, setTemplatesOpen] = useState(true);
  const render = (parentId = null, depth = 0) => folders.filter((f) => f.parentId === parentId).map((f) => (
    <div key={f.id} style={{ paddingLeft: depth * 12, marginTop: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, alignItems: "center" }}>
        <span style={{ cursor: "pointer", fontWeight: currentFolder === f.id ? 800 : 500, minWidth: 0, flex: 1 }} onClick={() => setCurrentFolder(f.id)} title={f.name}>{depth ? "▸ " : ""}{f.name}</span>
        <RowMenu items={[{ label: "Open", onClick: () => setCurrentFolder(f.id) }, { label: "Rename", onClick: () => onRename(f.id) }, { label: "Delete", onClick: () => onDelete(f.id) }]} />
      </div>
      {render(f.id, depth + 1)}
    </div>
  ));
        ...
        {templatesOpen ? <div style={{ marginTop: 8, display: "grid", gap: 6 }}>
          {templates.length ? templates.map((tpl) => (
            <div key={tpl.id} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, alignItems: "center" }}>
              ...
              <RowMenu items={[{ label: "Apply to current note", onClick: () => onApplyTemplate?.(tpl) }, { label: "New note from template", onClick: () => onNewFromTemplate?.(tpl) }, { label: "Delete template", onClick: () => onDeleteTemplate?.(tpl.id) }]} />
            </div>
