...
export default function PublicPage(props) {
  ...
  // available supplies request basket
  const [supplyBasket, setSupplyBasket] = useState({});
  ...
  const availableSupplies = useMemo(() => {
    return (Array.isArray(publicInventory) ? publicInventory : []).filter((it) => {
  ...
  function setSupplyQty(item, rawValue) {
    const key = supplyKey(item);
    if (!key) return;
    const n = Number(rawValue);
    setSupplyBasket((prev) => {
      const next = { ...(prev || {}) };
      if (!Number.isFinite(n) || n <= 0) delete next[key];
      else next[key] = Math.max(1, Math.floor(n));
      return next;
    });
  }
  ...
  const selectedSupplies = useMemo(() => {
    return availableSupplies
      .map((item) => {
        const key = supplyKey(item);
        const requestedQty = Number((supplyBasket || {})[key] || 0);
        return requestedQty > 0 ? { ...item, requestedQty } : null;
      })
      .filter(Boolean);
  }, [availableSupplies, supplyBasket]);
  ...
  async function submitInventoryRequest() {
    ...
    if (!selectedSupplies.length) {
      setInvReqStatus("Select at least one supply.");
      return;
    }

    const contact = String(invReqEmail || "").trim();
    const msg = String(invReqMsg || "").trim();

    const itemLines = selectedSupplies.map((item) => {
      const requested = `${item.requestedQty}${item.unit ? ` ${item.unit}` : ""}`;
      const available = item.qty != null && item.qty !== "" ? `${item.qty}${item.unit ? ` ${item.unit}` : ""}` : "unknown";
      const meta = [item.category, item.location].filter(Boolean).join(" · ");
      return `- ${item.name || "item"}: request ${requested}${meta ? ` (${meta})` : ""}; available ${available}`;
    });

    const details = [
      "available supplies request",
      ...itemLines,
      msg ? `message: ${msg}` : "",
    ].filter(Boolean).join("\n");
    ...
    try {
      ...
      setInvReqStatus("Request sent. The org will see it in the public inbox.");
      setSupplyBasket({});
      setInvReqMsg("");
      setInvReqName("");
      setInvReqEmail("");
    ...
    <div className="bf-public">
      ...
      <div className="bf-public-grid">
        ...
        {availableSupplies.length > 0 ? (
          <section className="bf-card">
            <div className="bf-card-header">
              <h3 className="bf-card-title">Available Supplies</h3>
              <p className="bf-card-hint">Select what you need and send one request.</p>
            </div>

            <ul className="bf-list">
              {availableSupplies.map((it) => {
                const key = supplyKey(it);
                const requestedQty = Number((supplyBasket || {})[key] || 0);
                return (
                  <li key={it.id || it.name} className="bf-li">
                    ...
                    <div className="bf-public-actions" style={{ alignItems: "center" }}>
                      <label className="bf-note" style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 0 }}>
                        ...
                        <input
                          ...
                          onChange={(e) => setSupplyQty(it, e.target.value)}
                          placeholder="1"
                          style={{ width: 88, padding: "8px 10px" }}
                        />
                      </label>
                      <button
                        className={requestedQty > 0 ? "bf-btn bf-btn-red" : "bf-btn"}
                        type="button"
                        onClick={() => {
                          setInvReqStatus("");
                          if (requestedQty > 0) setSupplyQty(it, "");
                          else setSupplyQty(it, 1);
                        }}
                      >
                        {requestedQty > 0 ? "Remove" : "Add to request"}
                      </button>
                    </div>
                  </li>
                );
            ...
            {selectedSupplies.length > 0 ? (
              <>
                ...
                <div className="bf-card" style={{ padding: 12 }}>
                  <div className="bf-card-header">
                    <h3 className="bf-card-title">Request selected supplies</h3>
                    <p className="bf-card-hint">{selectedSupplies.length} item{selectedSupplies.length === 1 ? "" : "s"} selected</p>
                  </div>

                  <div className="bf-form-grid">
                    <div className="bf-note" style={{ display: "grid", gap: 6 }}>
                      {selectedSupplies.map((item) => (
                        <div key={`selected-${supplyKey(item)}`} style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                          <span>{item.name || "item"}</span>
                          <span>{item.requestedQty}{item.unit ? ` ${item.unit}` : ""}</span>
                        </div>
                      ))}
                    </div>
              ...
              <div className="bf-note" style={{ marginTop: 12 }}>Select one or more supplies to start a request.</div>
            )}
          </section>
        ) : null}
