...
export default function Studio() {
	...
	const [workspaceSize, setWorkspaceSize] = React.useState({ width: 1200, height: 820 });
	...
	React.useEffect(() => {
		const measure = () => {
			...
			if (!node) return;
			setWorkspaceSize({ width: node.clientWidth || 1200, height: node.clientHeight || 820 });
		};
		measure();
	...
	const fitZoomPercent = React.useMemo(() => {
		if (!currentDoc) return 100;
		const availableWidth = Math.max(320, workspaceSize.width - 120);
		const availableHeight = Math.max(320, workspaceSize.height - 120);
		return Math.max(10, Math.min(100, Math.floor(Math.min(availableWidth / currentDoc.width, availableHeight / currentDoc.height) * 100)));
	}, [currentDoc, workspaceSize]);
	const renderScale = zoomPercent / 100;

	React.useEffect(() => {
		if (currentDoc) setZoomPercent(fitZoomPercent);
	}, [currentId, fitZoomPercent]);
	...
	const shellCss = `
		#bf-studio-shell { color: #f8fafc; }
		#bf-studio-shell button { border-radius: 14px; border: 1px solid rgba(255,255,255,0.10); background: linear-gradient(180deg, rgba(127,29,29,0.95), rgba(91,18,18,0.95)); color: white; padding: 10px 14px; font-weight: 700; box-shadow: 0 10px 24px rgba(0,0,0,0.18); }
		#bf-studio-shell button:disabled { opacity: 0.45; cursor: not-allowed; box-shadow: none; }
		#bf-studio-shell input, #bf-studio-shell textarea, #bf-studio-shell select { border-radius: 12px; border: 1px solid rgba(255,255,255,0.10); background: rgba(15,23,42,0.92); color: white; padding: 10px 12px; }
		#bf-studio-shell hr { border: 0; border-top: 1px solid rgba(255,255,255,0.08); margin: 16px 0; }
	`;

	return (
		<div id="bf-studio-shell" style={{ padding: 16, display: "grid", gridTemplateColumns: "240px minmax(0,1fr) 320px", gap: 16, background: "linear-gradient(180deg, #030712 0%, #02050d 100%)" }}>
			<style>{shellCss}</style>
			<div style={{ background: "linear-gradient(180deg, rgba(10,15,30,0.97), rgba(6,10,20,0.97))", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 22, padding: 16, boxShadow: "0 20px 40px rgba(0,0,0,0.25)" }}>
			...
			<div style={{ minWidth: 0 }}>
				...
				<div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 18, padding: 18, overflow: "auto", minHeight: 760 }} onMouseDown={() => setSelectedId(null)}>
					{currentDoc ? (
						<div style={{ width: currentDoc.width * renderScale, height: currentDoc.height * renderScale, margin: "24px auto", background: "#111827", borderRadius: 24, boxShadow: "0 45px 120px rgba(0,0,0,0.55)", overflow: "hidden", position: "relative", border: "1px solid rgba(255,255,255,0.06)" }}>
							<div id="bf-studio-canvas" style={{ width: currentDoc.width, height: currentDoc.height, transform: `scale(${renderScale})`, transformOrigin: "top left", position: "relative", background: "linear-gradient(180deg, #1f2937 0%, #0f172a 100%)" }}>
