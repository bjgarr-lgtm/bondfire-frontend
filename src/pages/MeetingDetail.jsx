      ...
      <div className="grid" style={{ gap: 10, marginTop: 12 }}>
        <div className="card" style={{ padding: 12 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ fontWeight: 800 }}>RSVPs</div>
            <div className="helper" style={{ opacity: 0.9 }}>
              Your RSVP: {myRsvp?.status ? String(myRsvp.status) : "none"}
            </div>
            <div style={{ flex: 1 }} />
            {rsvpMsg ? (
          ...
          <div className="grid cols-3" style={{ gap: 8, marginTop: 10 }}>
            <div className="card" style={{ padding: 10 }}>
              ...
              <div style={{ fontSize: 24, fontWeight: 800 }}>{Number(m?.rsvp_counts?.combined?.yes || 0)}</div>
            </div>
            <div className="card" style={{ padding: 10 }}>
              <div className="helper">Maybe</div>
              <div style={{ fontSize: 24, fontWeight: 800 }}>{Number(m?.rsvp_counts?.combined?.maybe || 0)}</div>
            </div>
            <div className="card" style={{ padding: 10 }}>
              <div className="helper">No</div>
              <div style={{ fontSize: 24, fontWeight: 800 }}>{Number(m?.rsvp_counts?.combined?.no || 0)}</div>
            </div>
          </div>
          <div className="helper" style={{ marginTop: 8 }}>
            Total RSVPs: {Number(m?.rsvp_counts?.combined?.total || 0)}
            {Number(m?.rsvp_counts?.public?.total || 0) > 0 ? ` • Public: ${Number(m?.rsvp_counts?.public?.total || 0)}` : ""}
            {Number(m?.rsvp_counts?.member?.total || 0) > 0 ? ` • Org members: ${Number(m?.rsvp_counts?.member?.total || 0)}` : ""}
          </div>
