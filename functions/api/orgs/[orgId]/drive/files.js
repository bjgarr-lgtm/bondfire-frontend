...
export async function onRequestPost({ env, request, params }) {
  ...
  async function persistBinary({ name, mime, parentId, bytes, size }) {
    const storageKey = `${orgId}/drive/files/${id}/${encodeURIComponent(name)}`;
    const bucket = getDriveBucket(env);
    const isEditable = isEditableTextMime(mime, name);
    ...
    if (bucket && storageKey) {
      try {
        ...
        if (!isEditable) {
          return bad(500, "FILE_STORAGE_FAILED", { detail: String(err?.message || err || "Drive bucket write failed") });
        }
      }
    }
    ...
    if (!bucket || !storageKey) {
      ...
      const textContent = isEditable ? textFromBytes(bytes) : "";
      await saveFileBlob(env, { orgId, fileId: id, storageKey, mime, dataUrl, textContent });
    }

    const createdFile = await getFileRecord(env, orgId, id, { includeData: isEditable });
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    const size = Number(upload.size || bytes.byteLength || 0);
    return persistBinary({ name, mime, parentId, bytes, size });
  }

  const rawName = request.headers.get("x-drive-name");
  if (rawName) {
    const name = String(rawName || "file").trim() || "file";
    const mime = String(request.headers.get("content-type") || request.headers.get("x-drive-mime") || "application/octet-stream");
